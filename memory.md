...
| 2026-06-29 | onCreated writes application_tab_id before user navigates to the new tab | background.ts onCreated fires while activeTabIdRef.current still points to the offer tab. handleStorageChange must NOT check activeTabIdRef when application_tab_id is set — it will always mismatch. The activeTabId useEffect is the right place: it runs on every tab switch and reads storage fresh, so it catches the match when the user actually arrives on the form tab. |
| 2026-06-29 | onUpdated URL-change fires on initial page load, not just form submission | The first chrome.tabs.onUpdated with changeInfo.url on the application tab is the form page LOADING, not submitting. Without a guard, this nulls out application_tab_id and sends FORM_SUBMITTED immediately. Fix: store form_url_set: false on onCreated; first URL change sets it to true (form loaded); only subsequent URL changes are treated as submission. |
| 2026-06-29 | Content script must never call chrome.storage.local directly | chrome.storage is undefined in content script context when extension context is invalidated. All storage reads/writes are proxied through background SW via safeSendMessage (GET_PENDING_APPLICATION, SET_PENDING_APPLICATION). Content script only calls chrome.runtime.sendMessage — never chrome.storage. |
