      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          {match.claude_score !== null && (
            <span
              className={`text-xs font-medium px-1.5 py-0.5 rounded ${scoreBadgeClass(match.claude_score)}`}
            >
              {match.claude_score}%
            </span>
          )}
        ...
        <div className="px-3 pb-3 flex flex-col gap-3 border-t border-gray-100">
          ...
          {isGenerating ? (
            ...
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-medium py-2 px-3 rounded-md text-sm transition-colors flex items-center justify-center"
            >
              Generate CV
            </button>
          )}
...
export default function MatchList({
  ...
  if (matches.length === 0) {
    return (
      <p className="text-sm text-gray-500">No pending matches for this offer</p>
    );
  }
